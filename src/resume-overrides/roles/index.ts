...
export const ROLE_VARIANTS: Record<string, RoleVariant> = {
  ...
  aviation: {
    ...
    experienceBullets: {
      0: ['Built 3 AI use cases on Huawei Ascend 310B NPU — optimized inference pipelines for internal workflow automation.', 'Developed smart piano hardware: motor driver circuit (LCEDA), STM32 firmware, oscilloscope/logic analyzer testing.', 'Built CV pipeline for piano finger gesture recognition on Ascend NPU; filed 2 patents from internship projects.'],
    ...
    experienceBulletsZh: {
      0: ['基于华为昇腾 310B NPU 开发 3 个 AI 应用场景，优化推理管线，延迟降低约 30%。', '参与智能钢琴硬件开发：全桥电机驱动电路设计（LCEDA）、STM32 固件、示波器/逻辑分析仪测试。', '搭建昇腾 NPU 上的手势识别 CV 管线；实习期间提交 2 项专利。'],
